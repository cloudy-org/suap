...
def package(
    ...
    platform_arch: Annotated[PlatformArch, Option()],
    ...
    if project == ProjectType.CARGO:
        ...
        toolchain_name = get_cargo_toolchain(platform_format, platform_arch)

        if toolchain_name is None:
            logger.error(
                "We don't support a cargo toolchain for this platform format " \
                    f"({platform_format.name}) and the architecture ({platform_arch.name}) yet!"
            )
            raise Exit(1)
        ...
        if platform_format & PlatformFormat.LINUX_BIN:
            ...
            format_and_copy_binary_to_dist(
                ...
                binary_suffix = f"linux-{platform_arch.value}",
        ...
        if platform_format & PlatformFormat.WINDOWS_BIN:
            ...
            format_and_copy_binary_to_dist(
                binary_path,
                binary_name = binary_name,
                binary_suffix = f"win-{platform_arch.value}.exe",
                dist_folder_path = dist_folder_path,
            )
        ...
        if platform_format & PlatformFormat.WINDOWS_SETUP:
            ...
            if platform_arch == PlatformArch.X86_64:
                ...
                format_config_and_make_nsis_installer(
                    binary_path,
                    binary_name = binary_name,
                    binary_suffix = f"win-{platform_arch.value}-setup.exe",
                    dist_folder_path = dist_folder_path,
                ...
                logger.error(
                    "Skipped making windows setup installer as it only supports x86_64!"
                )
